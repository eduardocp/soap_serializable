...
abstract class DecodeHelper implements HelperCore {
  ...
  CreateFactoryResult createFactory(Map<String, FieldElement> accessibleFields, Map<String, String> unavailableReasons) {
    ...
    _buffer.write('$targetClassReference '
        '${prefix}FromSoap${genericClassArgumentsImpl(true)}'
        '($mapType soap) {\n');

    String deserializeFun(String paramOrFieldName, {ParameterElement ctorParam}) => _deserializeForField(accessibleFields[paramOrFieldName], ctorParam: ctorParam);
  ...
  return \$checkedNew(
    ...
    () {\n''');
      ...
      data = _writeConstructorInvocation(element, accessibleFields.keys, accessibleFields.values.where((fe) => !fe.isFinal).map((fe) => fe.name).toList(), unavailableReasons, deserializeFun);
    ...
    final val = ${data.content};''');
      ...
      for (final field in data.fieldsToSet) {
        ...
        _buffer.write('''
    \$checkedConvert(soap, $safeName, (v) => ''');
      ...
      final fieldKeyMap = Map.fromEntries(data.usedCtorParamsAndFields.map((k) => MapEntry(k, nameAccess(accessibleFields[k]))).where((me) => me.key != me.value));

      String fieldKeyMapArg;
      if (fieldKeyMap.isEmpty) {
        fieldKeyMapArg = '';
      } else {
        final mapLiteral = soapMapAsDart(fieldKeyMap);
  ...
  void _writeChecks(int indent, SoapSerializable classAnnotation, Iterable<FieldElement> accessibleFields) {
    final args = <String>[];

    String constantList(Iterable<FieldElement> things) => 'const ${soapLiteralAsDart(things.map(nameAccess).toList())}';
    ...
    final requiredKeys = accessibleFields.where((fe) => soapKeyFor(fe).required).toList();
    ...
    final disallowNullKeys = accessibleFields.where((fe) => soapKeyFor(fe).disallowNullValue).toList();
  ...
  String _deserializeForField(FieldElement field, {ParameterElement ctorParam, bool checkedProperty}) {
    ...
    try {
      if (config.checked) {
        ...
        if (!checkedProperty) {
          value = '\$checkedConvert(soap, $soapKeyName, (v) => $value)';
        }
      } else {
        assert(!checkedProperty, 'should only be true if `_generator.checked` is true.');

        value = contextHelper.deserialize(targetType, 'soap[$soapKeyName]').toString();
      }
    } on UnsupportedTypeError catch (e) {
      throw createInvalidGenerationError('fromSoap', field, e);
    }

    final soapKey = soapKeyFor(field);
    final defaultValue = soapKey.defaultValue;
    if (defaultValue != null) {
      if (!contextHelper.nullable) {
        throwUnsupported(field, 'Cannot use `defaultValue` on a field with `nullable` false.');
      }
      if (soapKey.disallowNullValue && soapKey.required) {
      ...
      if (contextHelper.deserializeConvertData != null) {
        log.warning('The field `${field.name}` has both `defaultValue` and '
            '`fromSoap` defined which likely won\'t work for your scenario.\n'
            'Instead of using `defaultValue`, set `nullable: false` and handle '
            '`null` in the `fromSoap` function.');
      }
...
_ConstructorData _writeConstructorInvocation(ClassElement classElement, Iterable<String> availableConstructorParameters, Iterable<String> writableFields, Map<String, String> unavailableReasons,
    String deserializeForField(String paramOrFieldName, {ParameterElement ctorParam})) {
  final className = classElement.name;

  final ctor = classElement.unnamedConstructor;
  if (ctor == null) {
    // TODO(kevmoo): support using another ctor - dart-lang/soap_serializable#50
