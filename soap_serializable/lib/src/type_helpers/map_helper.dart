...
class MapHelper extends TypeHelper<TypeHelperContextWithConfig> {
  ...
  String serialize(DartType targetType, String expression, TypeHelperContextWithConfig context) {
    ...
    final subKeyValue = _forType(keyType)?.serialize(keyType, _keyParam, false) ?? context.serialize(keyType, _keyParam);
  ...
  String deserialize(DartType targetType, String expression, TypeHelperContextWithConfig context) {
    ...
    if (!isKeyStringable) {
      ...
      if (!context.nullable && (valueArgIsAny || simpleSoapTypeChecker.isAssignableFromType(valueArg))) {
...
void _checkSafeKeyType(String expression, DartType keyArg) {
  ...
  final safeKey = _isObjectOrDynamic(keyArg) || coreStringTypeChecker.isExactlyType(keyArg) || _isKeyStringable(keyArg);

  if (!safeKey) {
    throw UnsupportedTypeError(keyArg, expression, 'Map keys must be one of: ${_allowedTypeNames.join(', ')}.');
  }
}
