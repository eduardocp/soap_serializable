...
/// corresponding to `SoapSerializableGenerator` settings.
abstract class TypeHelperContextWithConfig extends TypeHelperContext {
  SoapSerializable get config;
}
...
abstract class TypeHelper<T extends TypeHelperContext> {
  ...
  /// Returns Dart code that deserializes an [expression] representing a SOAP
  /// literal to into [targetType].
  ///
  /// If [targetType] is not supported, returns `null`.
  ///
  /// Let's say you want to deserialize a class `Foo` by taking an `int` stored
  /// in a SOAP literal and calling the `Foo.fromInt` constructor.
  ///
  /// Treating [expression] as a opaque Dart expression representing a SOAP
...
Object commonNullPrefix(bool nullable, String expression, Object unsafeExpression) => nullable ? '$expression == null ? null : $unsafeExpression' : unsafeExpression;
