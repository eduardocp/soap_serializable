...
/// Helper function used in generated `fromSoap` code when
/// `SoapSerializable.disallowUnrecognizedKeys` is true for an annotated type or
/// `SoapKey.required` is `true` for any annotated fields.
///
...
void $checkKeys(Map map, {List<String> allowedKeys, List<String> requiredKeys, List<String> disallowNullValues}) {
  ...
  if (map != null && disallowNullValues != null) {
    final nullValuedKeys = map.entries.where((entry) => disallowNullValues.contains(entry.key) && entry.value == null).map((entry) => entry.key as String).toList();
...
/// Exception thrown if there are unrecognized keys in a SOAP map that was
...
class UnrecognizedKeysException extends BadKeyException {
  ...
  @override
  String get message => 'Unrecognized keys: [${unrecognizedKeys.join(', ')}]; supported keys: '
      '[${allowedKeys.join(', ')}]';

  UnrecognizedKeysException(this.unrecognizedKeys, Map map, this.allowedKeys) : super._(map);
}

/// Exception thrown if there are missing required keys in a SOAP map that was
/// provided during deserialization.
...
/// Exception thrown if there are keys with disallowed `null` values in a SOAP
